using BydTools.Wwise.Ww2ogg.Exceptions;

namespace BydTools.Wwise.Ww2ogg;

public class BitOggStream : System.IDisposable
{
    ...
    private uint payload_bytes;
    private bool first;
    private bool continued;
    private byte[] page_buffer = new byte[
        (int)AnonymousEnum.header_bytes
            + (int)AnonymousEnum.max_segments
            + (int)AnonymousEnum.segment_size * (int)AnonymousEnum.max_segments
    ];
    private uint granule;
    private uint seqno;
    ...
    public void put_bit(bool bit)
    {
        if (bit)
        {
            bit_buffer |= (byte)(1 << (int)bits_stored);
        }
    ...
    public void flush_bits()
    {
        if (bits_stored != 0)
        {
            if (
                payload_bytes
                == (uint)AnonymousEnum.segment_size * (uint)AnonymousEnum.max_segments
            )
            {
                flush_page(true);
            ...
            page_buffer[
                (int)AnonymousEnum.header_bytes + (int)AnonymousEnum.max_segments + payload_bytes
            ] = bit_buffer;
            payload_bytes++;
    ...
    {
        if (payload_bytes != (uint)AnonymousEnum.segment_size * (uint)AnonymousEnum.max_segments)
        {
            flush_bits();
        }
        ...
        {
            uint segments =
                (payload_bytes + (uint)AnonymousEnum.segment_size)
                / (uint)AnonymousEnum.segment_size; // intentionally round up
            if (segments == (uint)AnonymousEnum.max_segments + 1)
            {
                segments = (uint)AnonymousEnum.max_segments; // at max eschews the final 0
            }

            // move payload back
            for (uint i = 0; i < payload_bytes; i++)
            {
                page_buffer[(int)AnonymousEnum.header_bytes + segments + i] = page_buffer[
                    (int)AnonymousEnum.header_bytes + (int)AnonymousEnum.max_segments + i
                ];
            }

            page_buffer[0] = (byte)'O';
            page_buffer[1] = (byte)'g';
            page_buffer[2] = (byte)'g';
            ...
            {
                if (bytes_left >= (uint)AnonymousEnum.segment_size)
                {
                    bytes_left -= (uint)AnonymousEnum.segment_size;
                    page_buffer[27 + i] = (byte)AnonymousEnum.segment_size;
                }
                else
                {
                    page_buffer[27 + i] = (byte)bytes_left;
                }
            }

            // checksum
            EndianReadWriteMethods.Write32LE(
                page_buffer,
                22,
                checksum(
                    page_buffer,
                    (int)AnonymousEnum.header_bytes + (int)segments + (int)payload_bytes
                )
            );
    ...
    /* from Tremor (lowmem) */
    internal static uint[] crc_lookup =
    {
        0x00000000,
        0x04c11db7,
        0x09823b6e,
